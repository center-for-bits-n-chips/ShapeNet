...
class DigitalDisplay:
    ...
    def display_positions(self):
        ...
        if (self.mocap_server.config.NN_enable or self.mocap_server.config.LS_enable) and self.last_coefficients is not None:
            print("\n=== Neural Network Coefficients ===")
            for i, coef in enumerate(self.last_coefficients):
                    print(f"Coefficient {i+1}: {self.format_value(coef)}")
            print("\n=== Nadir ===")
            print(f"X: {self.nadir_x:.2f}")
            print(f"Y: {self.nadir_y:.2f}")
            print(f"Z: {self.nadir_z:.2f}")
    ...
    def update_nadir(self, nadir_x, nadir_y, nadir_z):
        self.nadir_x = nadir_x
        self.nadir_y = nadir_y
        self.nadir_z = nadir_z